...
## if run with python 2, make sure input of float type
...
#USE_IGNORE_SEGMENT_ENDINGS = False
...
class Event:
    ...
    def __init__(self, type, point, segment, slope, original_segment=None):
...
class SweepLine:
    ...
    def get_intersections_index(self):
        insect = []
        for k in self.intersections:
            ind = []
            for item in self.intersections[k]:
                ind.append(item.original_segment[1])
            if ind[0] > ind[1]:
                ind = [ind[1], ind[0]]
            insect.append([k, ind])
        return insect
...
class EventQueue:
    ...
    def __init__(self, segments, line, original_segments=None):
        ...
        for i, s in enumerate(segments):
            ...
            if s[0] == s[1]:
                ...
                e_start = Event(Event.Type.START_VERTICAL, s[0], s, slope, original_segments[i])
                ...
                e_start = Event(Event.Type.START, s[0], s, slope, original_segments[i])
                e_end   = Event(Event.Type.END,   s[1], s, slope, original_segments[i])
...
def isect_segments(segments, show_index=False):
    ## keep original point sequence
    seg_original = [[s, i] for i, s in enumerate(segments)]
    ...
    queue = EventQueue(segments, sweep_line, seg_original)
    ...
    if not show_index:
        ...
        return sweep_line.get_intersections_index()


#def isect_polygon(points) -> list:
def isect_polygon(points):
    n = len(points)
    segments = [
