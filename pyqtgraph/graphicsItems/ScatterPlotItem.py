...
def drawSymbol(painter, symbol, size, pen, brush):
    ...
    if isinstance(symbol, basestring):
        try:
            numsymbol = int(symbol)
            font = QtGui.QFont()
            font.setBold(True)
            font.setWeight(10)
            font.setPixelSize(1)
            painter.drawPath(Symbols['o'])
            mid = (len(symbol) - 1.0) / 2
            for i, num in enumerate(symbol):
                textPath = QtGui.QPainterPath()
                textPath.addText(QtCore.QPoint(0, 0), font, num)
                tr = QtGui.QTransform()
                tr.translate(-0.25 + (i - mid) * 0.3, 0.25)
                tp = tr.map(textPath)
                painter.drawPath(tp)
    ...
    **Signals:**
    ...
    def hoverEvent(self, ev):
        ...
        if hover:
            pts = self.pointsAt(ev.pos())
            if len(pts) > 0:
                #print "hover on point: ", pts
                #self.ptsClicked = pts
                self.sigHoverEvent.emit((self, pts))
                #ev.accept()
            else:
                #print "no spots"
                #ev.ignore()
